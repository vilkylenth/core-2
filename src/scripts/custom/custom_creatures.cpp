...
bool GossipHello_TeleportNPC(Player *player, Creature *_Creature)   
{
    ...
    if (player->GetTeam() == HORDE)
    {
        ...
        player->ADD_GOSSIP_ITEM(5, "团本"                 , GOSSIP_SENDER_MAIN, 101);
        player->ADD_GOSSIP_ITEM(5, "古拉巴什竞技场" , GOSSIP_SENDER_MAIN, 4015);
        player->ADD_GOSSIP_ITEM(5, "区域 - 卡利姆多"     , GOSSIP_SENDER_MAIN, 6010);
        player->ADD_GOSSIP_ITEM(5, "区域 - 东部王国"     , GOSSIP_SENDER_MAIN, 6020);
...
// ---### ZONES ###---
        ...
        case 100: // Main Menu
            ...
            else
            {
                //player->ADD_GOSSIP_ITEM(5, "PreTBC Mall"          , GOSSIP_SENDER_MAIN, 74);
                player->ADD_GOSSIP_ITEM(5, "主城"               , GOSSIP_SENDER_MAIN, 2);
                player->ADD_GOSSIP_ITEM(5, "出生点"       , GOSSIP_SENDER_MAIN, 4);
                player->ADD_GOSSIP_ITEM(5, "副本"            , GOSSIP_SENDER_MAIN, 5);
                player->ADD_GOSSIP_ITEM(5, "团本"                 , GOSSIP_SENDER_MAIN, 101);
                player->ADD_GOSSIP_ITEM(5, "古拉巴什竞技场" , GOSSIP_SENDER_MAIN, 4015);
                player->ADD_GOSSIP_ITEM(5, "区域 - 卡利姆多"     , GOSSIP_SENDER_MAIN, 6010);
...
void Enchant(Player* player, Item* item, uint32 enchantid)
{
    if (!item)
    {
        player->GetSession()->SendNotification("你必须先装备你想要附魔的物品.");
        return;
    }

    if (!enchantid)
    {
        player->GetSession()->SendNotification("出错.");
        return;
    }
...
bool GossipSelect_EnchantNPC(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    if (action < 20)
    {
        ...
        case EQUIPMENT_SLOT_HANDS:
            ...
            player->ADD_GOSSIP_ITEM(5, "冰霜能量", GOSSIP_SENDER_MAIN, GLOVES_FROST);
        ...
        case EQUIPMENT_SLOT_FEET:
            ...
            player->ADD_GOSSIP_ITEM(5, "初级速度", GOSSIP_SENDER_MAIN, BOOTS_SPEED);
        ...
        case EQUIPMENT_SLOT_MAINHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "单手 敏捷", GOSSIP_SENDER_MAIN, WEP1H_AGILITY);
            player->ADD_GOSSIP_ITEM(5, "双手 敏捷", GOSSIP_SENDER_MAIN, WEP2H_AGILITY);
        ...
        case EQUIPMENT_SLOT_OFFHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "冰霜反射", GOSSIP_SENDER_MAIN, OFFHAND_FROSTRES);
            break;
        }
...
bool LearnAllRecipesInProfession(Player *pPlayer, SkillType skill)
{
    ...
    if (!SkillInfo)
    {
        sLog.outError("专业 NPC: 收到无效的技能ID");
        return false;
    }

    pPlayer->SetSkill(SkillInfo->id, 300, 300);
    LearnSkillRecipesHelper(pPlayer, SkillInfo->id);
    pPlayer->GetSession()->SendNotification("所有 %s 的配方已学习 ", skill_name);
    return true;
}
...
bool GossipHello_ProfessionNPC(Player* player, Creature* creature)
{
    ...
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_2, "铁匠",        GOSSIP_SENDER_MAIN, 2);
    ...
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_2, "挖矿",               GOSSIP_SENDER_MAIN, 11);
